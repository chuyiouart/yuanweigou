...
class DailySiteIntegrationTests(unittest.TestCase):
    ...
    def test_client_keeps_current_links_same_origin_and_maps_legacy_github_archive(self):
        ...
        self.assertIn('const sameOriginOuartUpdate =', script)
        self.assertIn('resolved.pathname === "/"', script)
        self.assertIn('resolved.searchParams.get("post_type") === "ouart_update"', script)
        self.assertIn('/^\\d+$/.test(resolved.searchParams.get("p") || "")', script)
        self.assertIn('safeProtocol && sameOriginOuartUpdate', script)
